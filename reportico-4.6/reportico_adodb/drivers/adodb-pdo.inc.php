...
//10601b
// I deleted class adob_pdo_base from hear and inserted after class adodb_pdo
//		( Run Project with SQLite-3 was not working )
//10601e
...
class ADODB_pdo extends ADOConnection {
	...
	function _UpdatePDO()
	{
		...
		$this->_genIDSQL = 0; // $d->_genIDSQL;
		$this->_genSeqSQL = $d->_genSeqSQL;
		$this->_dropSeqSQL = 0; //$d->_dropSeqSQL;
    ...
    // Native PDO Error Message for Mssql is standard message, on error
    // seelct true message for error no from database
	function ErrorMsgDBLIB($errorno)
    {
        $arr = array();
        $stmt = $this->_query("SELECT * from sys.messages where message_id = $errorno", $arr);
        if ( $stmt )
        {
            $row = $stmt->fetch();
            if ( $row )
	...
	function ErrorMsg()
	{
		if ($this->_errormsg !== false && $this->dsnType == "dblib" ) 
        {
            $msg = $this->ErrorMsgDBLIB($this->_errorno);
            if ( $msg ) 
                return $msg;
        }
...
class ADODB_pdo_base extends ADODB_pdo {
	...
	function _init($parentDriver)
	{
		...
		#$parentDriver->_connectionID->setAttribute(PDO::mysqli_ATTR_USE_BUFFERED_QUERY,true);
	}

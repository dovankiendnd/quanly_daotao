...
$sql = "SELECT  admission_number AS ADM_NO , `sirname`, `firstname`, `lastname`, `dateofbirth`, `gender`, `mobile`,reg_date AS Admission_Date, `border` FROM `studentstable` WHERE course_id=1";
$resultset = mysqli_query($db, $sql) or die("database error:". mysqli_error($db));


$sql2 = "SELECT COUNT(gender) AS nu FROM studentstable WHERE course_id=1 AND gender='Male' ";
...
$sql3 = "SELECT COUNT(gender) AS nu FROM studentstable WHERE course_id=1 AND gender='Female' ";
...
$sql3 = "SELECT COUNT(border) AS borders FROM studentstable WHERE course_id=1 AND border='Border' ";
$resultset3 = mysqli_query($db, $sql3) or die("database error:". mysqli_error($db));
while($users3=mysqli_fetch_array($resultset3))
        {        
...
$pdf->SetXY(100,25);
$pdf->Cell(100,5,'Form 1 Students',0,0,'C',0);
//C MEANS CENTERED

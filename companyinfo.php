<!--this are php codes to fetch data from the database. the data is later displayed in the fields provided-->
<?php
error_reporting(0);
...
$id = 1;
    ...
    $qry=mysqli_query($conn,$select);
        ...
        while($rec = mysqli_fetch_array($qry)){
            $cname = "$rec[cname]";
            $cemail = "$rec[cemail]";
            $ccontact = "$rec[ccontact]";
            $clocation = "$rec[clocation]";
            $clogo = "$rec[clogo]";
...
<!--this is the heading section-->   
    <h2>
            <?php
            $query=mysqli_query($conn,"select * from companyinfo");
...
<!--end of heading section-->  
 ...
 <div style="float:left;"></div></div><div style="float:right;"> Current User: <font color="orange">John</font> <a href="xxx">Log Out<img src="assets/img/logout.png"></a></div><hr>
    ...
    <div class="row">
        ...
        <div class="col-sm-6">
            <div>
                <form action="updatecompanyinfo.php" method="post" enctype="multipart/form-data">
                    <label>Company name</label>
                    <input type="text" name="cname" required class="form-control" value="<?php echo $cname ?>"/><br/>

                    <label>Email address</label>
                    <input type="text" name="cemail" required class="form-control" value="<?php echo $cemail ?>"/><br/>

                    <label>Contact</label>
                    <input type="text" name="ccontact" required class="form-control" value="<?php echo $ccontact ?>"/><br/>

                    <label>Location</label>
                    <input type="text" name="clocation" required class="form-control" value="<?php echo $clocation ?>"/><br/>
                    
                    <label>Logo (82px x 82px)</label>
                    <input type="file" name="clogo" required class="form-control" /><br/>
                    


                    <input type="submit" name="register" class="btn btn-primary" value="Update Details">
                </form>
            </div>
        </div>            
